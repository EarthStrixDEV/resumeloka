...
export default function ResumeLokaLanding() {
  ...
  const STEPS = [
    { n: "01", icon: <Upload size={20} />, title: t("step1Title"), text: t("step1Text") },
    { n: "02", icon: <ScanLine size={20} />, title: t("step2Title"), text: t("step2Text") },
    { n: "03", icon: <Briefcase size={20} />, title: t("step3Title"), text: t("step3Text") },
  ];

  const FEATURES = [
    { icon: <FileSearch size={18} />, cls: "f-brand", title: t("feat1Title"), text: t("feat1Text") },
    { icon: <MessagesSquare size={18} />, cls: "f-tip", title: t("feat2Title"), text: t("feat2Text") },
    { icon: <Target size={18} />, cls: "f-good", title: t("feat3Title"), text: t("feat3Text") },
    { icon: <Zap size={18} />, cls: "f-warn", title: t("feat4Title"), text: t("feat4Text") },
    { icon: <ShieldCheck size={18} />, cls: "f-brand", title: t("feat5Title"), text: t("feat5Text") },
    { icon: <TrendingUp size={18} />, cls: "f-good", title: t("feat6Title"), text: t("feat6Text") },
  ];

  const PAINS = [t("pain1"), t("pain2"), t("pain3")];

  const STATS = [
    { num: "7 sec", label: t("stat1Label") },
    { num: "0–100", label: t("stat2Label") },
    { num: "2", label: t("stat3Label") },
  ];

  const REVIEWS = [
    { quote: t("review1Quote"), name: "Tanawat S.", role: "Frontend Developer" },
    { quote: t("review2Quote"), name: "Ploy R.", role: "Marketing Specialist" },
    ...
    <div className="lp">
      ...
      <section className="hero" id="top">
        <div className="hero-copy">
          <span className="pill"><span className="pill-dot" /> {t("heroPill")}</span>
          <h1 className="hero-title">
            {t("heroTitle1")}<span className="hl">{t("heroHighlight")}</span>.<br />{t("heroTitle2")}
          </h1>
          <p className="hero-sub">
            {t("heroSubPre")}<b>JobThai</b>{t("heroSubAnd")}<b>JobsDB</b>{t("heroSubPost")}
          </p>
          <div className="hero-actions">
            <a href="/analyzer" className="btn btn-primary btn-lg">
              {t("heroCta")} <ArrowRight size={17} />
            </a>
            <a href="#how" onClick={go("how")} className="btn btn-ghost btn-lg">{t("heroHowLink")}</a>
          </div>
          <div className="hero-trust">
            <div className="stars">{[0, 1, 2, 3, 4].map((i) => <Star key={i} size={15} fill="currentColor" strokeWidth={0} />)}</div>
            <span><b>4.8</b>{t("heroTrustMid")}<b>12,000+</b>{t("heroTrustEnd")}</span>
          </div>
        </div>
      ...
      <section className="strip">
        ...
        <div className="strip-logos">
          ...
          <span className="strip-more">{t("stripMore")}</span>
        </div>
      </section>

      {/* how it works */}
      <section className="section" id="how">
        <Reveal><p className="eyebrow">{t("howEyebrow")}</p></Reveal>
        <Reveal delay={60}><h2 className="section-title">{t("howTitle")}</h2></Reveal>
        <div className="steps">
          {STEPS.map((s, i) => (
      ...
      <section className="section pain-section">
        <div className="pain-grid">
          <Reveal className="pain-left">
            <p className="eyebrow">{t("painEyebrow")}</p>
          ...
          <Reveal delay={120} className="pain-right">
            <div className="promise-card">
              ...
              <p>{t("promiseText")}</p>
              <a href="/analyzer" className="btn btn-primary">{t("promiseCta")} <ArrowRight size={16} /></a>
            </div>
          </Reveal>
        </div>
      </section>
      ...
      <section className="section" id="features">
        <Reveal><p className="eyebrow">{t("featEyebrow")}</p></Reveal>
        <Reveal delay={60}><h2 className="section-title">{t("featTitle")}</h2></Reveal>
      ...
      <section className="section" id="reviews">
        <Reveal><p className="eyebrow">{t("reviewEyebrow")}</p></Reveal>
        <Reveal delay={60}><h2 className="section-title">{t("reviewTitle")}</h2></Reveal>
      ...
      <section className="section faq-section">
        ...
        <Reveal delay={60}><h2 className="section-title">{t("faqTitle")}</h2></Reveal>
      ...
      <section className="cta-band" id="cta">
        <Reveal>
          <div className="cta-inner">
            <h2>{t("ctaTitle")}</h2>
            <p>{t("ctaText")}</p>
            <a href="/analyzer" className="btn btn-light btn-lg">
              <Upload size={17} /> {t("ctaCta")}
            </a>
            <span className="cta-fine">{t("ctaFine")}</span>
          </div>
        </Reveal>
      </section>
    ...
    <div className="mock">
      ...
      <div className="mock-body">
        <div className="mock-score">
          ...
          <div>
            <p className="mock-k">{t("appMockScore")}</p>
            <p className="mock-h">{t("appMockStrong")}</p>
          </div>
        </div>

        <div className="mock-chips">
          <span className="chip good"><ThumbsUp size={12} /> {t("appMockAdv")}</span>
          <span className="chip warn"><AlertTriangle size={12} /> {t("appMockFix")}</span>
          <span className="chip tip"><Lightbulb size={12} /> {t("appMockTips")}</span>
        </div>
      ...
      <span className="float-tag tag-a"><Check size={12} strokeWidth={3} /> {t("appMockJobThai")}</span>
      <span className="float-tag tag-b"><Zap size={12} /> {t("appMockAnalyzed")}</span>
    </div>
  );
}
...
.nav{position:sticky; top:0; z-index:50; display:flex; align-items:center; justify-content:space-between;
  padding:14px 30px; background:rgba(255,255,255,.82); backdrop-filter:blur(12px); border-bottom:1px solid var(--line); gap:12px;}
...
.nav-links{display:flex; align-items:center; gap:26px; flex:1; justify-content:flex-end;}
...
@media(max-width:680px){
  .nav-links{position:absolute; top:62px; left:0; right:0; flex-direction:column; align-items:stretch; gap:4px;
    background:var(--paper); border-bottom:1px solid var(--line); padding:14px 20px 18px; display:none; flex:unset;}
